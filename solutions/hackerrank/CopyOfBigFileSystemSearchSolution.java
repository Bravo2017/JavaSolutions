...
public class CopyOfBigFileSystemSearchSolution {
	...
	public static void main(String[] args) {
		CopyOfBigFileSystemSearchSolution testSystem1 = new CopyOfBigFileSystemSearchSolution();
	...
	private int queryAll(HashMap<Integer, Integer> queryMap) {
		...
		for (int fileIndex = 0; fileIndex < inputFiles.length; ++fileIndex) {
			...
			for (Map.Entry<Integer, Integer> query : queryMap.entrySet()) {
				Integer queryKey = query.getKey();
				Integer queryValue = query.getValue();
				
				Integer fileValue = ithFileMap.get(queryKey);
				if(fileValue == null || fileValue < queryValue){
					query_all_flag = false;
					break;
				}
			}
			//for current file, 
			if (query_all_flag)		resultQueryAll++;
		}
	...
	private int queryAny(HashMap<Integer, Integer> queryMap) {
		...
		for (int fileIndex = 0; fileIndex < inputFiles.length; ++fileIndex) {
			...
			if (query_one_flag)		resultQueryAny++;
		}
	...
	private int querySome(HashMap<Integer, Integer> queryMap) {
		...
		for (int fileIndex = 0; fileIndex < inputFiles.length; ++fileIndex) {
			...
			for (Map.Entry<Integer, Integer> query : queryMap.entrySet()) {
				...
				if (fileValue == null || fileValue < queryValue) {
					query_all_flag = false;
					if (query_one_flag || fileValue != null)
						break;
				} else
					query_one_flag = true;
			}
			if (!query_all_flag && query_one_flag)	resultQuerySome++;
		}
...
class QueryFile {
	...
	public QueryFile(String[] stringToken) {
		...
		int tempKey = 0;
		Integer prevListing = null;
		for (int i = 0; i < this.length; ++i) {
			tempKey = Integer.parseInt(stringToken[i + 2]);
		
			//get previous count, get returns null, if key not present
			prevListing = mapQueryNumber.get(tempKey);
...
class DataFile {
	...
	public DataFile(int[] array, int length) {
		...
		int key = 0;
		Integer prevListing = 0;
		for (int i = 0; i < length; ++i) {
			key = array[i];
			prevListing = mapFileNumber.get(key);
