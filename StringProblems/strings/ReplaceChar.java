...
/**
 * replaceLetter() used a char[] for all string manipulations, replaceLetter2()
 * uses string method string.charAt(). both build a new char array with modified
 * chars Performance for a very long string, replaceLetter() has twice better
 * performce(half time) to do the operation,
 * 
 * Algorithmically, replaceLetter() is 3xO(n) replaceLetter2() is 2xO(n)
 */
...
public class ReplaceChar {
	...
	/*
	 * public static void replaceLetters(String someString){ int length =
	 * someString.length(); char[] arr = someString.toCharArray(); //for faster
	 * performance - O(n)
	 * 
	 * //scan the string once to find the number of spaces int _spaces = 0;
	 * for(int i = 0 ; i < length ; ++i){ // O(n) if(arr[i] == ' ') ++_spaces; }
	 * 
	 * //scanning and replacing - O(n) int newLength = length + 2*_spaces;
	 * char[] arrNew = new char[newLength];
	 * 
	 * for(int i = 0, j = 0 ; ( i < length && j < newLength ); ++i){ if(arr[i]
	 * == ' '){ arrNew[j++] = '%'; arrNew[j++] = '2'; arrNew[j++] = '0'; }else{
	 * arrNew[j++] = arr[i]; } } //return new String(arrNew); }
	 */
