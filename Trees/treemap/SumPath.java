...
public class SumPath<K extends Comparable<K>, V> 
				extends BinarySearchTreeMap<K, V> {
	
	public void printPathWithSum(int sum){
		Node<K,V>[] path = (Node<K,V>[])new Node[100];
		int pathIndex = 0;
		printPathWithSum(this.root, sum, path, pathIndex);
	}
	
	public void printPathWithSum(Node<K,V> x, int sum, 
								 Node<K,V>[] path, int pathIndex){
		if(x == null){
			return;
		}
		sum = sum - (int)x.key;
