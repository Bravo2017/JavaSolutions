...
public class LinkedListDetectAndRemoveLoops{
	...
	private void findAndRemoveLoop(Node x) {
		...
		nodes.add(ref_curr); //add the first node - head (check for null - since hashSet allows for one null element)
...
//				break;	//not necessary - as ref becomes null in the following line
				return;	//exit from the method- break wont work as we do not want to print anything if there is a loop
			}
