...
public class LinkedListLoop {
	...
	public void buildList() {
		...
		Node b = new Node("b", a);
		Node c = new Node("a", b); // new node, duplicate data, since its a new
									// node object -its not dup
		Node d = new Node("d", c);
		Node e = new Node("e", d);
		...
		// System.out.println(c.hashCode()); // Node c has different hashCode
		// from Node a.
	...
	private boolean testCircularWithSet(Node x) {
		// Use a set to add all elements - in case of a cycle- stop at first dup
		// elements
		// Issue - if same objects are present in the list, at different
		// positions
		// then Set might report duplicate, best is to override equals() such
		// that
		// the reference of next node is also considered when looking for
		// identical nodes
		// Observation - default equals() method will take care of this, since
		// even if
		...
		if (refNode == null)
			return false; // List not circular
		return true;// List circular
	}

	/**
	 * Floyds Cycle-Finding Algorithm: Returns true if loop found
	 */
